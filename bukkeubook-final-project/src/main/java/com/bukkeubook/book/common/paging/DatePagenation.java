...
public class DatePagenation {
	
	/* 검색어가 없는 경우 페이징 처리만을 위한 용도 */
	public static DateSelectCriteria getDateSelectCriteria(int pageNo, int totalCount, int limit, int buttonAmount) {
		
		return getDateSelectCriteria(pageNo, totalCount, limit, buttonAmount, null, null);
	}
	
	/* 검색어가 존재하는 경우 검색 조건으로 select 후 페이징 처리를 하기 위한 용도 */
	public static DateSelectCriteria getDateSelectCriteria(int pageNo, int totalCount, int limit, int buttonAmount, java.sql.Date startDate, java.sql.Date endDate) {
...
//		startPage = (((int)((double) pageNo / buttonAmount + 0.9)) - 1) * buttonAmount + 1;
		...
		DateSelectCriteria DateSelectCriteria = new DateSelectCriteria(pageNo, totalCount, limit, buttonAmount ,maxPage, startPage, endPage, startRow, endRow, startDate, endDate);
		
		return DateSelectCriteria;
	}
}
